...
function isMissingAvailabilityTable(error) {
    return (
        ...
        error?.code === "42703" ||
        error?.code === "PGRST204" ||
        /business_hours|business_breaks|schedule_blocks|barber_/i.test(
            error?.message || ""
        )
    )
}
...
function overlaps(hour, ranges) {
    const value = toMinutes(hour)
    return ranges.some((range) => {
        const start = toMinutes(range.starts_at)
        const end = toMinutes(range.ends_at)
        return value >= start && value < end
    })
}

function fallbackSchedule(date, occupiedHours) {
    return FALLBACK_HOURS.map((hour) => {
        const [scheduleHour] = hour.split(":")
        const isPast = dayjs(date).add(Number(scheduleHour), "hour").isBefore(dayjs())

        return {
            hour,
            available: !occupiedHours.includes(hour) && !isPast,
        }
    })
}

async function fetchOccupiedHours({ date, barberId, excludeAppointmentId }) {
    ...
    if (error && barberId && isMissingAvailabilityTable(error)) {
    ...
    return (data || [])
        ...
        .map((appointment) => dayjs(appointment.when_at).format("HH:mm"))
}
...
function generateConfiguredSchedule({ date, occupiedHours, rules }) {
...
async function getDailyAvailability({ date, barberId = "", excludeAppointmentId = "" }) {
    const occupiedHours = await fetchOccupiedHours({
        date,
        barberId,
    ...
    if (!rules) {
        return fallbackSchedule(date, occupiedHours)
    }

    return generateConfiguredSchedule({ date, occupiedHours, rules })
}

async function isSlotAvailable({ when, barberId = "", excludeAppointmentId = "" }) {
